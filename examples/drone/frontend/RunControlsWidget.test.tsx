...
describe("RunControls widget", () => {
  ...
  it("publishes a start command to drone/control", async () => {
    const { socket } = await renderWidget();

    await page.getByRole("button", { name: "Start manoeuvre run" }).click();

    expect(published(socket)).toContainEqual({ command: "start" });
  });

  it("publishes a preset selection to drone/control", async () => {
    const { socket } = await renderWidget();

    await page.getByRole("button", { name: "Apply gentle preset" }).click();

    expect(published(socket)).toContainEqual({ preset: "gentle" });
  });

  it("reflects the run phase/verdict from drone/state", async () => {
    const { socket } = await renderWidget();

    act(() =>
      socket.deliver("drone/state", {
        phase: "done",
        verdict: "FAIL",
        message: "ok",
      }),
    ...
    await expect.element(page.getByText("Done (FAIL)")).toBeInTheDocument();
  });
});

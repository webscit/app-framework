...
to attach the ``POST /api/ai/layout`` endpoint to a FastAPI application.
"""
...
class LayoutRequest(BaseModel):
    """Request body for ``POST /api/ai/layout``.
...
class LayoutResponse(BaseModel):
    """Response body for ``POST /api/ai/layout``.
...
def mount_ai_routes(app: FastAPI) -> None:
    ...
    Defines a ``POST /ai/layout`` route on a fresh :class:`~fastapi.APIRouter`
    and includes it on ``app`` under :data:`API_PREFIX`, so the endpoint ends
    up at ``/api/ai/layout``.
    ...
    @router.post("/ai/layout", response_model=LayoutResponse)
